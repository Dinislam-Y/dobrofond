...
class DocTest():
    ...
    # driver.find_element('name', 'Вход').click()
    ...
    driver.find_element('xpath', '//td[text()="Первый Человек Семьи"]').click()
    time.sleep(time_costom)

    driver.find_element('xpath', '//button[text()="Документы"]').click()
    time.sleep(time_costom)

    driver.find_element('xpath', '//button[text()="Добавить Документы"]').click()
    time.sleep(time_costom)

    name_document = driver.find_element('name', 'name')
    name_document.click()
    name_document.clear()
    name_document.send_keys('test doc')
    time.sleep(4)

    driver.find_element('xpath', '//button[text()="Выберите файл"]').click()
    time.sleep(4)


class NegativDocTest():
    url = 'https://admin.dobrofon.org/login'
    driver.get(url)
    time_costom = 0.5
